            ...
            <TabView class="mb-3">
                ...
                <TabPanel header="دخول بالبريد الالكتروني">
                    <inputs-form-control mode="form-control" ref="mail" type="email" id="email" v-model.trim="email"
                        @keypress="emptyPhone"> البريد
                        الالكتروني
                    </inputs-form-control>
                </TabPanel>
            </TabView>
            <inputs-form-control mode="form-control" type="password" id="password1" v-model.trim="password"> كلمة
                المرور </inputs-form-control>
            <NuxtLink :to="localePath('/login/email')" class="main_color text-start d-block  "> هل نسيت كلمة المرور ؟
            </NuxtLink>
...
export default {
    ...
    methods: {
        async login() {
            try {
                await useAuthStore().signIn({ email: this.email, password: this.password});
